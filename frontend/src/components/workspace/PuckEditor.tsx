...
export default function PuckEditor({ workspaceId, initialData, architectureData, workspaceName }: Props) {
  ...
  useEffect(() => {
    const isInternalEmpty = !internalData.content || internalData.content.length === 0;
    if (!initialData && isInternalEmpty && architectureData?.nodes?.length) {
      setInternalData(architectureToDesign(architectureData, workspaceName ?? ''));
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [architectureData?.nodes?.length, initialData]);

  // Debounced autosave — only when data actually changed from what was loaded
  useEffect(() => {
    if (!internalData.content || internalData.content.length === 0) return;

    const id = setTimeout(() => {
      // Basic check to avoid redundant saves
      const hasChanged = JSON.stringify(internalData.content) !== JSON.stringify(initialData?.content);
      if (hasChanged) {
