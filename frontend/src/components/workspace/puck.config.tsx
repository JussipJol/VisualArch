  ...
  components: {
    ...
    FeatureGrid: {
      fields: {
        features: {
          type: "array",
          getItemSummary: (item: any, idx: number) => item?.title || `Feature ${idx + 1}`,
          arrayFields: {
            title: { type: "text" },
            description: { type: "textarea" },
          },
      ...
      render: ({ features }) => (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 py-8">
          {features.map((f, i) => (
            <div key={`feature-${i}-${f.title}`} className="p-6 rounded-2xl bg-surface border border-white/5 space-y-2">
    ...
    Pricing: {
      fields: {
        tiers: {
          type: "array",
          getItemSummary: (item: any, idx: number) => item?.plan || `Tier ${idx + 1}`,
          arrayFields: {
            plan: { type: "text" },
            price: { type: "text" },
      ...
      render: ({ tiers }) => (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 py-12">
          {tiers.map((t, i) => (
            <div key={`tier-${i}-${t.plan}`} className="p-8 rounded-3xl bg-surface border border-white/5 space-y-6 relative overflow-hidden group">
              ...
              <ul className="space-y-3">
                {t.features.map((f: any, fi: number) => (
                  <li key={`feature-${fi}-${f.item}`} className="text-sm text-text-secondary flex items-center gap-2">
      ...
      render: ({ logo, links }) => (
        <nav className="flex items-center justify-between py-6 px-4 border-b border-white/5 mb-8">
          ...
          <div className="flex items-center gap-6">
            {links.map((l, i) => (
              <a key={`link-${i}-${l.label}`} href={l.href} className="text-sm font-medium text-text-secondary hover:text-accent transition-colors">
                {l.label}
              </a>
            ))}
          </div>
