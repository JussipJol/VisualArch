...
export const WorkspacePage = () => {
  ...
  useEffect(() => {
    ...
    // Локальный проект — берём из localStorage
    if (localProjects.isLocalId(id)) {
      const local = localProjects.getById(id);
      if (local) {
        setProject(local as Parameters<typeof setProject>[0]);
        setLoading(false);
      } else {
  ...
  return (
    <div style={{ width: '100vw', height: '100vh', background: '#000', color: '#fff', fontFamily: '"Space Mono", monospace', display: 'flex', flexDirection: 'column', overflow: 'hidden', position: 'relative' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', padding: '0 20px', height: 52, borderBottom: '1px solid rgba(255,255,255,0.06)', flexShrink: 0, gap: 16 }}>
        ...
        <div style={{ flex: 1 }} />
        <span style={{ fontSize: '0.65rem', color: 'rgba(255,255,255,0.18)', letterSpacing: 1, fontFamily: '"Space Mono", monospace', borderLeft: '1px solid rgba(255,255,255,0.06)', paddingLeft: 16 }}>{user?.email}</span>
      </div>
